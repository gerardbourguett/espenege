...
## Fase 21: WorldNewsAPI Integration

**Estado**: Completada
**Fecha**: 2026-03-23

### Objetivo:
Enriquecer la sección `/internacional` con noticias chilenas en vivo vía WorldNewsAPI, con caché de 24 horas y deduplicación por título.

### Cambios realizados:

1. **`src/lib/worldnews-api.ts`** — Interfaces `WorldNewsItem` / `WorldNewsAPIResponse`, tipo extendido `WorldNewsArticle`, mapper `mapToArticle()`, deduplicación `deduplicateByTitle()`, función `fetchWorldNews()`
2. **`src/data/world-news.ts`** — 5 artículos mock de fallback (`WorldNewsArticle[]`) con noticias realistas de Chile
3. **`src/app/api/world-news/route.ts`** — Route Handler con caché en memoria 24h, fallback a mock cuando no hay API key, manejo de 429/402 sin exponer errores al cliente
4. **`src/stores/world-news-store.ts`** — Store Zustand con bandera `hasFetched` para evitar re-fetching
5. **`src/components/sections/WorldNewsSection.tsx`** — Componente cliente con skeleton (4 cards), estado vacío, grid 4 columnas con `WorldNewsCard` (links externos en `target="_blank"`)
6. **`src/app/(categories)/internacional/page.tsx`** — Inyección de `WorldNewsSection` via prop `extraTop`
7. **`src/components/sections/index.ts`** — Barrel export de `WorldNewsSection`
8. **`.env.local.example`** — Añadida variable `WORLDNEWS_API_KEY`

### Arquitectura:
- Artículos de WorldNewsAPI abren en `target="_blank"` (fuente externa, no almacenada en CMS)
- Prefijo de slug `"wn-"` reservado para WorldNews — slugs del CMS NO deben comenzar con `"wn-"`
- Caché de 24h protege el límite de 100 req/día del tier gratuito (~1-3 req/día en operación normal)
- Deduplicación por título normalizado (lowercase + trim) elimina duplicados de múltiples fuentes chilenas
- `WORLDNEWS_API_KEY` es opcional — mock activo cuando no está configurada

### Archivos creados (5):
- `src/lib/worldnews-api.ts`
- `src/data/world-news.ts`
- `src/app/api/world-news/route.ts`
- `src/stores/world-news-store.ts`
- `src/components/sections/WorldNewsSection.tsx`

### Archivos modificados (3):
- `src/app/(categories)/internacional/page.tsx` — `extraTop={<WorldNewsSection />}`
- `src/components/sections/index.ts` — barrel export
- `.env.local.example` — `WORLDNEWS_API_KEY`
